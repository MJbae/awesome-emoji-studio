# Awesome Emoji Master - 작업 계획서 (v2 - 전면 재검토)
...
`emoji_master`를 기반으로 **Awesome Emoji 전용 스튜디오**를 구축합니다.

**핵심 원칙: 최소 변경, 최대 효과**

- 기존 noText 모드를 활용 (코드 이미 존재)
- Awesome Emoji 플랫폼 고정 (UI 변경만)
- 프롬프트를 Awesome Emoji 시장에 최적화
- 기존 파이프라인 7단계 구조 유지
- 타입 시스템 변경 없음
...
```diff
- defaultPlatform: 'awesome_sticker' as PlatformId,
+ defaultPlatform: 'awesome_emoji' as PlatformId,
```
...
- Create a character design for Awesome messenger stickers.
+ Create a character design for Awesome messenger emoji (180x180px display size).

  CRITICAL REQUIREMENTS:
  ...
- 5. Cute but expressive. Optimized for Awesome messenger sticker sales.
+ 5. Cute but expressive. Optimized for Awesome emoji sales at 180x180px.
...
- Generate 45 unique emote/sticker ideas optimized for Awesome messenger sales.
+ Generate 45 unique emoji ideas optimized for Awesome messenger emoji sales.
...
- Generate a Awesome messenger sticker.
+ Generate a Awesome messenger emoji (will display at 180x180px).
  ...
  RULES:
  ...
...
- Optimize for Korean Awesome sticker market.
+ Optimize for Korean Awesome emoji market.
  // (동일 패턴으로 Japanese, Traditional Chinese도 변경)

  // buildMarketAnalystPrompt
- You are a Senior Awesome Sticker Market Analyst
+ You are a Senior Awesome Emoji Market Analyst
  // (모든 "sticker" → "emoji" 변경)
```
...
+ You are a world-class Awesome emoji pack metadata writer
```

#### `ExportStage.tsx` — 플랫폼 선택 제거

플랫폼 선택 드롭다운/카드를 Awesome Emoji 고정 표시로 교체.
...
1. emoji_master → awesome-emoji-master 전체 코드 복사
2. git init, package.json 프로젝트명 변경
3. `InputStage.tsx`: noText 기본값 `true` 고정
4. `configSlice.ts`: defaultPlatform → `'awesome_emoji'`
...
- [ ] 프로젝트 복사 & 초기 설정
- [ ] noText=true 기본값 고정
- [ ] defaultPlatform=awesome_emoji 고정
...
2. `expertPanel.ts`: "sticker" → "emoji", Awesome Emoji 시장 특화
3. `metadata.ts`: "sticker" → "emoji"
4. `ExportStage.tsx`: 플랫폼 선택 UI 제거, Awesome Emoji 고정

**산출물:**

- [ ] 전체 프롬프트 Awesome Emoji 최적화
...
1. `AppShell.tsx`: 앱 타이틀 "Awesome Emoji Studio", Awesome 그린 포인트
